...
SavedGameState SaveManager::createSnapshot(const Hero& hero, const StoryGraph& story) const {
    ...
    state.visitedStoryNodes = story.getVisitedNodes();
...
bool SaveManager::applySnapshot(const SavedGameState& state, Hero& hero, StoryGraph& story) const {
    try {
        ...
        if (!state.visitedStoryNodes.empty()) {
            story.setVisitedNodes(state.visitedStoryNodes);
        }
...
bool SaveManager::saveGameState(int slot, const SavedGameState& state) const {
    ...
    try {
        ...
        storyJ["flags"] = flagsJ;
        storyJ["visitedNodes"] = state.visitedStoryNodes;
        j["story"] = storyJ;
        j["storyFlags"] = flagsJ; // Root fallback
        j["visitedNodes"] = state.visitedStoryNodes;
    ...
    try {
        ...
        if (j.contains("story") && j["story"].is_object()) {
            ...
            if (j["story"].contains("visitedNodes") && j["story"]["visitedNodes"].is_array()) {
                outState.visitedStoryNodes = j["story"]["visitedNodes"].get<std::vector<std::string>>();
            }
        } else {
        ...
        if (outState.visitedStoryNodes.empty() && j.contains("visitedNodes") && j["visitedNodes"].is_array()) {
            outState.visitedStoryNodes = j["visitedNodes"].get<std::vector<std::string>>();
        }
