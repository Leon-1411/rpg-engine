...
bool BossMonster::checkEnrage() {
    if (!enraged && isAlive() && hp <= static_cast<int>(maxHp * 0.3)) {
        triggerEnrage();
        return true;
    }
    return false;
...
void BossMonster::regenerate() {
    ...
    int oldHp = hp;
    heal(healAmount);
    std::cout << ">>> [BOSS REGENERATION] " << name << " recovers "
              << (hp - oldHp) << " HP (" << hp << "/" << maxHp << ")!\n";
}
...
void BossMonster::displayStats() const {
    ...
    std::cout << "Status: " << (isAlive() ? (enraged ? "ENRAGED (Berserk)" : "Normal") : "DEFEATED") << "\n";
